...
""".format(args=self,name=self.name())
    ...
    def run_eb_and_ee(self):  
        ...
        subprocess.Popen(["bin/$SCRAM_ARCH/RegressionTrainerExe",self.cfg_name()]).communicate()
        ...
        subprocess.Popen(["bin/$SCRAM_ARCH/RegressionApplierExe",self.input_testing,self.applied_name(),"--gbrForestFileEE",forest_ee_file,"--gbrForestFileEB",forest_eb_file,"--nrThreads","4","--treeName",self.tree_name,"--writeFullTree",self.write_full_tree,"--regOutTag",self.reg_out_tag]).communicate()
