...
def main():
    ...
    if args.era=='2016':
        ...
        era_name = "Run3HLT"
        input_ideal_ic  = "{}/output_1.root".format(args.input_dir)
        input_real_ic = "{}/output_1.root".format(args.input_dir)
        ideal_eventnr_cut = "evt.eventnr%10000==0"  #4million electrons (we determined 4 million was optimal but after the 2017 was done)
        real_eventnr_cut = "evt.eventnr%10000==1" #4million electrons (we determined 4 million was optimal but after the 2017 was done)
        ep_eventnr_cut = "evt.eventnr%10000==2" #4million electrons (we determined 4 million was optimal but after the 2017 was done)
#        ideal_eventnr_cut = "evt.eventnr%5==0"  #4million electrons (we determined 4 million was optimal but after the 2017 was done)
#        real_eventnr_cut = "evt.eventnr%5==1" #4million electrons (we determined 4 million was optimal but after the 2017 was done)
#        ep_eventnr_cut = "evt.eventnr%5==2" #4million electrons (we determined 4 million was optimal but after the 2017 was done)
    else:
        raise ValueError("era {} is invalid, options are 2016/2017/2018/Run3".format(era))
    ...
    if run_step2: subprocess.Popen(["bin/slc7_amd64_gcc900/RegressionApplierExe",input_real_ic,input_for_res_training,"--gbrForestFileEE",forest_ee_file,"--gbrForestFileEB",forest_eb_file,"--nrThreads","4","--treeName",regArgs.tree_name,"--writeFullTree","1","--regOutTag","Ideal"]).communicate()
